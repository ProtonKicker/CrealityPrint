...
void MainFrame::init_menubar_as_editor()
{
    ...
    // 创建水平布局的容器面板
    wxPanel* menuPanel = new wxPanel(this);
    ...
    // 添加文件菜单到菜单栏
    menuSizer->Add(fileMenu, 0, wxALIGN_CENTER_VERTICAL);
    
    // 准备菜单
    wxMenu* prepareMenu = new wxMenu;
    // 添加准备菜单项...
    ...
    m_menubar->Append(prepareMenu, _L("Prepare"));
    m_menubar->Append(previewMenu, _L("Preview"));
    m_menubar->Append(deviceMenu, _L("Device"));

    // 设置菜单栏居中对齐
    m_menubar->SetWindowStyle(m_menubar->GetWindowStyle() | wxALIGN_CENTER_HORIZONTAL);
    // （此处添加各个菜单项的具体内容）
    
    // 将菜单添加到菜单栏，并居中
    menuSizer->Add(prepareMenu, 0, wxALIGN_CENTER);
    menuSizer->Add(previewMenu, 0, wxALIGN_CENTER);
    menuSizer->Add(deviceMenu, 0, wxALIGN_CENTER);
    
    // 设置菜单栏的布局
    menuPanel->SetSizer(menuSizer);
    m_menubar->SetWindowStyle(wxMB_DOCKABLE);
    m_menubar->SetSizeHints(menuPanel);
    m_menubar->SetSizerAndFit(menuSizer);
    
    // 绑定尺寸事件以动态调整菜单栏位置
    Bind(wxEVT_SIZE, [this](wxSizeEvent& event) {
        wxSize size = event.GetSize();
        // 动态计算并设置菜单栏的位置
        int x = size.GetWidth() / 2 - m_menubar->GetSize().GetWidth() / 2;
        int y = 0; // 根据需要调整垂直位置
        m_menubar->SetPosition(wxPoint(x, y));
        event.Skip();
    });
